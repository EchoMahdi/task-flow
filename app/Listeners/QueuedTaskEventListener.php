...
/**
 * Queued Task Event Listener
 * 
 * Handles task-related events asynchronously via queue.
 * Implements ShouldQueue for background processing.
 * 
 * @package  App\Listeners
 */
class QueuedTaskEventListener implements ShouldQueue
{
    /**
    ...
    {
        Log::info('Task created via queued event system', [
            'taskId' => $event->taskId,
    ...
    {
        Log::info('Task updated via queued event system', [
            'taskId' => $event->taskId,
    ...
    {
        Log::info('Task deleted via queued event system', [
            'taskId' => $event->taskId,
            'projectId' => $event->projectId,
            'source' => $event->source,
        ]);
    ...
    {
        Log::info('Task completion status changed via queued event system', [
            'taskId' => $event->taskId,
    ...
    {
        Log::info('Task project assignment changed via queued event system', [
            'taskId' => $event->taskId,
            'projectId' => $event->projectId,
