...
/**
 * Queued Team Event Listener
 * 
 * Handles team-related events asynchronously via queue.
 * Implements ShouldQueue for background processing.
 * 
 * @package  App\Listeners
 */
class QueuedTeamEventListener implements ShouldQueue
{
    /**
    ...
    {
        Log::info('Team created via queued event system', [
            'teamId' => $event->teamId,
            'name' => $event->name,
    ...
    {
        Log::info('Team updated via queued event system', [
            'teamId' => $event->teamId,
    ...
    {
        Log::info('Team deleted via queued event system', [
            'teamId' => $event->teamId,
            'source' => $event->source,
        ]);
    }
    ...
    {
        Log::info('Member added to team via queued event system', [
            'teamId' => $event->teamId,
            'userId' => $event->userId,
            'role' => $event->role,
            'source' => $event->source,
    ...
    {
        Log::info('Member removed from team via queued event system', [
            'teamId' => $event->teamId,
            'userId' => $event->userId,
            'source' => $event->source,
        ]);
    ...
    {
        Log::info('Team member role changed via queued event system', [
            'teamId' => $event->teamId,
            'userId' => $event->userId,
            'role' => $event->role,
